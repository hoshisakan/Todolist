...
class UserViewset(viewsets.ModelViewSet):
    ...
    def create(self, request):
        try:
            ...
            send_email(
                subject='[Todo List] Activate Register Account',
                username=username,
    ...
    def reset_password(self, request):
        try:
            ...
            # 檢查密碼是否有被更改過，避免使用者藉由 access token 反覆重設密碼
    ...
    def login_validate(self, request):
        try:
            data = request.data
            queryset = self.viewset_func.obtainUsername(data['username'])
    ...
    def validate_register_activate(self, request):
        try:
            ...
            if activate_result != 1:
                raise Exception("The account activate failed")
    ...
    def apply_reset_password(self, request, *args, **kwargs):
        try:
            ...
            if not self.viewset_func.checkEmailIsExists(email=email):
                ...
                send_email(
                    subject='[Todo List] Reset Password Request',
                    username=username,
                    template='reset_password.html',
                    link=link,
                    to=[email]
                )
            return Response({'message': 'Reset Password Apply Success.','apply_reset_success': True}, status=status.HTTP_200_OK)
        except Exception as e:
